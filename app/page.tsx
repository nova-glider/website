...
export default function Home() {
  ...
  function contactModal(
    ...
    if (hasEmail) {
      contacts.push(
        `<a href='mailto:${email}' class='text-blue-400 underline hover:text-blue-300 transition-colors cursor-pointer'>${email}</a>`,
      );
    }
    if (hasPhone) {
      contacts.push(
        `<a href='tel:${phone}' class='text-blue-400 underline hover:text-blue-300 transition-colors cursor-pointer'>${phone}</a>`,
      );
    }
    if (hasUrl) {
      contacts.push(
        `<a href='${url}' target='_blank' rel='noopener noreferrer' class='text-blue-400 underline hover:text-blue-300 transition-colors cursor-pointer'>${url.replace(/^https?:\/\//, "")}</a>`,
      );
    }
  ...
  return (
    <>
      ...
      >
        <div className="w-full max-w-3xl backdrop-blur-md bg-white/5 p-6 sm:p-10 rounded-2xl border border-white/10 shadow-lg">
          ...
          <p className="text-base sm:text-lg text-gray-300 leading-relaxed">
            We are a team of creative and innovative students who are trying to
            build a CanSat that will deploy a glider mid-air during its descent.
            The goal of this mission is to provide a proof of concept for
            deploying multiple flying objects from a single satellite, which
            could have applications in atmospheric research, search and rescue
            operations, and more. The glider will come down autonomously without
            parachute and will collect and transmit data back to the ground
            station.
          </p>
        </div>
        <a
      ...
      <section
        id="cards"
        className="flex items-center justify-center p-10 bg-black relative overflow-hidden select-none"
      >
        {/* Particles.js container */}
        <div
            ...
            {cardsData.cards.map((card, i) => (
              <div key={i} className="w-full max-w-sm mx-auto">
                ...
                >
                  <div
                    id={`glow-overlay-${i}`}
                    className="absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-100"
                  ></div>
                  <Image
                    src={card.img}
                    alt="Profile pic"
                  ...
                  />
                  <h2 className="text-xl font-bold text-center text-white relative z-20">
                    {card.name}
                  </h2>
                  <p className="text-center text-blue-300 text-sm relative z-20">
                    {card.role}
                  </p>
                  <button
                    onClick={() => {
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 transition-opacity duration-300 opacity-100">
              <div className="bg-white/20 dark:bg-gray-900/60 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 p-6 sm:p-8 max-w-xs w-[90%] relative flex flex-col items-center">
                {modal.isHtml ? (
                  ...
                  <span className="text-white text-center text-lg font-semibold mb-6">
                    {modal.content}
                  </span>
                )}
                <button
      ...
      <footer className="bg-gradient-to-t from-black via-gray-950 to-gray-950 text-gray-500 py-6 text-center border-t border-white/10 px-4">
        <div className="max-w-2xl mx-auto flex flex-col items-center gap-2">
          <p className="text-sm sm:text-base font-medium tracking-wide">
            © 2025-2026 NovaGlider. All rights reserved.
          </p>
          <p className="text-xs mt-1 text-gray-600">
            Made with <span className="text-pink-400">❤️</span> by the
            NovaGlider team.
          </p>
          {/* Green hosting badge temporarily commented out */}
          {/* <img src="https://app.greenweb.org/api/v3/greencheckimage/www.novaglider.be?nocache=true" alt="This website runs on green hosting - verified by thegreenwebfoundation.org" width="100px" /> */}
        </div>
      </footer>
    </>
  );
